...
nn = Classifier(
	layers = [Layer('Rectifier', units = 1000), Layer('Rectifier', units = 400), Layer('Rectifier', units = 50), Layer('Softmax', units = 5)],
	learning_rate = 0.03,
	n_iter = 70,
	batch_size = 10
)
...
pipeline = Pipeline([
		#('PCA', PCA(n_components = 70, kernel = 'rbf')),
...
scores = cross_val_score(pipeline, X_train, y_train, cv = 10, scoring = 'accuracy', verbose = 4)
...
outfile = open('pred05_fmorath.csv','w')
